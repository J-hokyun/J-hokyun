...
### 다양한 개발경험을 중요시하는 개발자
AI 시대에는 하나의 기술을 깊게 파는 것만큼, 다양한 기술을 경험하는 것도 중요하다고 생각합니다. </br>
새로운 기술을 마주할 때마다 겉핥기에 그치지 않고, 동작 원리와 설계 의도까지 이해하며 개발하려고 노력하는 개발자입니다.
</div>
...
| **[🎧 O :D](https://github.com/J-hokyun/OD)** | 사용자 맞춤 음성 오디오북 플랫폼 | Backend | `Django 5` `DRF` `PostgreSQL` `AWS(EC2·S3·RDS)` `OAuth2` `JWT` `TTS·RVC` |
| **[📈 NH 빅데이터 경진대회](https://github.com/J-hokyun/NH_Competition)** | 한·미·중 섹터 주가 상관관계 분석 | 데이터 수집 · 중국 분석 | `Python` `pandas` `SciPy` `BeautifulSoup` `yfinance` |
| **[🏠 Togethers](https://github.com/J-hokyun/CapStone_NSU)** | 룸메이트 매칭 플랫폼 (캡스톤 디자인) | Backend | `Spring Boot 2.7` `JPA` `MySQL` `Thymeleaf` `JWT` `WebSocket` |
...
- 📌 약 3개월 운영 후, **수익구조가 부족하여 지속적인 운영이 불가능하다고 판단해 서비스를 종료**했습니다. 
...
- **AI 파이프라인 설계** — 학습과 변환을 분리하고 모델은 S3, 메타데이터는 RDS로 나눠 **재생 지연 최소화**
